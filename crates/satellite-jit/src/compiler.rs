...
impl JitCompiler {
    ...
    pub fn compile_constraint(
        &self,
        constraint: &crate::ir::JitConstraint,
        name: &str,
        ...
        builder
            ...
            .map_err(|e| Error::CompilationError(format!("Build return failed: {:?}", e)))?;
        ...
        if function.verify(true) {
            ...
            Err(Error::CompilationError(
                "Invalid LLVM IR generated".to_string(),
            ))
        }
    }
    ...
    fn build_expr<'a>(
        ...
        match expr {
            Var(idx) => {
                ...
                let ptr = unsafe {
                    builder
                        .build_gep(i8_type, assignments, &[idx_val], "var_ptr")
                        .map_err(|e| Error::CompilationError(format!("GEP failed: {:?}", e)))?
                };
                let val = builder
                    .build_load(i8_type, ptr, "val")
                    .map_err(|e| Error::CompilationError(format!("Load failed: {:?}", e)))?
                    .into_int_value();
                let one = i8_type.const_int(1, false);
                Ok(builder
                    .build_int_compare(inkwell::IntPredicate::EQ, val, one, "is_true")
                    .map_err(|e| Error::CompilationError(format!("Cmp failed: {:?}", e)))?)
            }
            Lit(lit) => {
                ...
                let ptr = unsafe {
                    builder
                        .build_gep(i8_type, assignments, &[idx_val], "lit_ptr")
                        .map_err(|e| Error::CompilationError(format!("GEP failed: {:?}", e)))?
                };
                let val = builder
                    .build_load(i8_type, ptr, "val")
                ...
                Ok(builder
                    .build_int_compare(inkwell::IntPredicate::EQ, val, target_val, "is_lit_true")
                    .map_err(|e| Error::CompilationError(format!("Cmp failed: {:?}", e)))?)
            }
            And(args) => {
                ...
                for arg in args {
                    ...
                    current = builder
                        .build_and(current, val, "and_acc")
                        .map_err(|e| Error::CompilationError(format!("And failed: {:?}", e)))?;
                }
                Ok(current)
            }
            Or(args) => {
                ...
                for arg in args {
                    let val = self.build_expr(builder, arg, assignments)?;
                    current = builder
                        .build_or(current, val, "or_acc")
                        .map_err(|e| Error::CompilationError(format!("Or failed: {:?}", e)))?;
                }
                Ok(current)
            }
            Not(arg) => {
                let val = self.build_expr(builder, arg, assignments)?;
                Ok(builder
                    .build_not(val, "not_res")
                    .map_err(|e| Error::CompilationError(format!("Not failed: {:?}", e)))?)
            }
            Xor(args) => {
                let mut current = bool_type.const_int(0, false);
                for arg in args {
                    let val = self.build_expr(builder, arg, assignments)?;
                    current = builder
                        .build_xor(current, val, "xor_acc")
                        .map_err(|e| Error::CompilationError(format!("Xor failed: {:?}", e)))?;
                }
                Ok(current)
            }
            AtMostK { args, k } => {
                ...
                for arg in args {
                    let val = self.build_expr(builder, arg, assignments)?;
                    let val_i32 = builder
                        .build_int_z_extend(val, self.context.i32_type(), "zext")
                        .map_err(|e| Error::CompilationError(format!("Zext failed: {:?}", e)))?;
                    sum = builder
                        .build_int_add(sum, val_i32, "sum_acc")
                        .map_err(|e| Error::CompilationError(format!("Add failed: {:?}", e)))?;
                }
                let k_val = self.context.i32_type().const_int(*k as u64, false);
                Ok(builder
                    .build_int_compare(inkwell::IntPredicate::ULE, sum, k_val, "at_most_k")
                    .map_err(|e| Error::CompilationError(format!("Cmp failed: {:?}", e)))?)
            }
